<?php
	...
	if( isset( $_POST["narea"] ) ){ 
		...
		if( nombreDisponible( $dbh, "area", "nombre", $area["nombre"], "", $area["idu"] ) ){
			...
			if( $rsp != 0 ){
				$res["exito"] = 1;
				$res["mje"] = "Area created successfully";
			}else{
				$res["exito"] = 1;
				$res["mje"] = "There was a problem creating area";
			}
		}
		else{ 
			$rsp = -2;
			$res["mje"] = "Area name already exists";
		}
	...
	if( isset( $_POST["earea"] ) ){ 
		...
		if( nombreDisponible( $dbh, "area", "nombre", $area["nombre"], $area["id"], "" ) ){
			...
			if( $rsp != 0 ){
				$res["exito"] = 1;
				$res["mje"] = "Area updated successfully";
			}else{
				$res["exito"] = 0;
				$res["mje"] = "There was a problem updating area";
			}
		}
		else{ 
			$rsp = -2;
	...
	if( isset( $_POST["elim_area"] ) ){
		...
		if( tienePropositosArea( $dbh, $ida ) ){
			$res["exito"] = -1;
			$res["mje"] = "Cannot delete area, there are purposes linked to area";
		}else{
			eliminarArea( $dbh, $ida );
			eliminarSO_Area( $dbh, $ida );
			$res["exito"] = 1;
			$res["mje"] = "Area deleted successfully";
		}
		echo json_encode( $res );
	}
	/* --------------------------------------------------------- */
