<?php
	...
	if( isset( $_POST["lyrpass"] ) ){ 
		...
		if( $rsp["valido"] ){
			...
			$res["exito"] = 1;
			$res["mje"] = "An email has been sent with your password";
		} else {
			$res["exito"] = 0;
			$res["mje"] = "There is no account linked to this email, check your info and try again";
		}
	...
	if( isset( $_POST["usr_reg"] ) ){ 
		...
		if( ( $rsp != 0 ) && ( $rsp != "" ) ){
			...
			$res["exito"] = 0;
			$res["mje"] = "There was a problem creating your account";
		}
		echo json_encode( $res );
	}
	/* --------------------------------------------------------- */
