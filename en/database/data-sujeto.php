<?php
	...
	if( isset( $_POST["nsujeto"] ) ){ 
		...
		if( nombreDisponible( $dbh, "sujeto", "nombre", $sujeto["nombre"], "", $sujeto["idu"] ) ){
			...
			if( $ids != 0 ){
				$res["exito"] = 1;
				$res["mje"] = "Subject created successfully";
				$res["reg"] = $sujeto;
			}else{
				$res["exito"] = -1;
				$res["mje"] = "There was a problem creating subject";
			}
		}else{ 
			$res["exito"] = -2;
			$res["mje"] = "Subject name already exists";
	...
	if( isset( $_POST["edit_sujeto"] ) ){ 
		...
		if( nombreDisponible( $dbh, "sujeto", "nombre", $sujeto["nombre"], $sujeto["id"], "" ) ){
			...
			if( $rsp != 0 ){
				$res["exito"] = 1;
				$res["mje"] = "Subject updated successfully";
			}else{
				$res["exito"] = 0;
				$res["mje"] = "There was a problem updating subject";
			}
		}
		else{ 
			$rsp = -2;
	...
	if( isset( $_POST["elim_sujeto"] ) ){
		...
		if( tienePropositosSujeto( $dbh, $ids ) ){
			...
			$res["exito"] = 1;
			$res["mje"] = "Subject deleted successfully";
		}
		echo json_encode( $res );
	}
	/* --------------------------------------------------------- */
